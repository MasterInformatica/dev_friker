...
public class CardBartok : Card { // Make sure CardBartok extends Card
    // This is how you use the enum CardState
    public CardState state = CardState.drawpile;
    // The hiddenBy list stores which other cards will keep this one face down
    public List<CardBartok> hiddenBy = new List<CardBartok>();
	...
	public override void OnMouseUpAsButton ()
	{
		if (state == CardState.drawpile) {
            Puntos.S.Totaliza();
			Bartok.S.newTarget(Bartok.S.Draw());
		} else if (state == CardState.tableau) {
			if (hiddenBy.Count > 0)
				return;
			if( Bartok.S.possibleTarget(this) ){
				Bartok.S.oneLessTableau(this);
                Puntos.S.addPoints();
                Bartok.S.newTarget(this);
			}
		} else {
